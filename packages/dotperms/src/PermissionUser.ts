import { Permission, UserPermissionSet, UserPermissions } from "."
	...
	public check(permission: Permission) {
		...
		switch (permTiers.length) {
			...
			case 2: {
				...
				if (
					(typeof subGroup === "boolean" && subGroup === true) ||
					subGroup === null ||
					!subGroup
				) {
					result = null
			...
			switch (split.length) {
				...
				case 2: {
					...
					if (typeof groupTwo !== "boolean") {
						// biome-ignore lint/style/noNonNullAssertion: Annoying to deal with rn
