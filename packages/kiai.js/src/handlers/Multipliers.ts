...
export class Multipliers extends BaseHandler {
	...
	async createMultiplier(guildId: string, data: Multiplier) {
		const result = (await this._handler.request(`/guild/${guildId}/multipliers`, "POST", {}, data)) as Message
		return result
	}

	async resetMultipliers(guildId: string) {
		const result = (await this._handler.request(`/guild/${guildId}/multipliers`, "DELETE")) as { channels: number; roles: number }
		return result
	}

	async getMultiplier(guildId: string, type: "channel" | "role" | "guild", id: string) {
		const result = await this._handler.request(`/guild/${guildId}/multipliers/${type}${type === "guild" ? "" : `/${id}`}`)
		switch (type) {
			case "channel":
	...
	async clearMultiplier(guildId: string, type: "channel" | "role" | "guild", id: string) {
		const result = (await this._handler.request(`/guild/${guildId}/multipliers/${type}${type === "guild" ? "" : `/${id}`}`, "DELETE")) as Message
		return result
	}
}
